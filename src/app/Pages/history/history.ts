...
export class History implements OnInit {
  ...
  private loadHistory() {
    ...
    if (!userId) {
      console.warn('[history] missing user id; skipping scan_results fetch');
      this.applyScans([]);
      return;
    }
    ...
    if (!isAdminUser) {
      ...
      console.info('[history] using token source:', tokenSource);

      this.fetchScans(userToken, userId, filterByUser).subscribe({
        next: (scans) => {
          console.info('[history] scan_results count:', scans.length);
          this.applyScans(scans);
        },
        error: (fetchErr) => {
          console.error('[history] scan_results error:', fetchErr);
        }
      });
      return;
    }
    ...
    this.adminTokens.getToken().subscribe({
      next: (adminToken) => {
        ...
        this.fetchScans(token, userId, filterByUser).subscribe({
          ...
          error: (err) => {
            console.error('[history] scan_results error:', err);
          }
        });
      },
      error: (err) => {
        console.error('[history] admin token error:', err);
        const tokenSource = userToken ? 'user' : 'none';
        console.info('[history] using token source:', tokenSource);
